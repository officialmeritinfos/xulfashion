...
@section('content')
    <div class="container-fluid">
        ...
        <form class="theme-form profile-setting" action="{{route('mobile.user.profile.settings.complete-profile.socialite.process')}}" method="post" id="basicSettings"
                ...
                <div class="form-input mb-4">
                    <select type="text" class="form-control" id="inputusername"  name="country">
                        ...
                        @foreach($countries as $country)
                            <option value="{{$country->iso3}}" {{ ($user->countryCode==$country->iso3)?'selected':''  }}>{{ $country->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group d-block mb-3">
                <label for="referral" class="form-label">Referral Code<sup><small>(Only if you were referred by anyone)</small></sup></label>
