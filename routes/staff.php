...
Route::domain('staff.localhost')->group(function () {
    ...
    Route::post('login/set-password/process',[LoginController::class,'processPasswordSetUp'])->name('staff.setup.password.process');

    //lockout account
    Route::get('login/{staff}/lock/account',[LoginController::class,'lockStaffAccount'])->name('staff.lock.account')->middleware('signed');

    //Staff dashboard main

    Route::middleware(['auth.staff','staff.login','auth:staff','auth.session'])->prefix('me')->group(function () {
